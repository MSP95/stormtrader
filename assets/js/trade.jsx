...
export default class Trade extends React.Component {
  ...
  componentWillReceiveProps(props) {
    ...
    for (let i = 0; i < stocks.length; i++) {
      obj.push({
        id: stock_name.indexOf(stocks[i]),
        name: stocks[i],
        qty: qty[stock_name.indexOf(stocks[i])]
      })
    }
  ...
  buyStock(event) {
    ...
    let buy_object = {
      ...
      qty: parseInt(stock_quantity),
      bought_at: this.props.stocksPrice[stock_id]
    }
    ...
    if (stock_name != "" && stock_quantity != "" && stock_id != -1) {
      this.channel.push("buy_request", {buy: send_object}).receive("ok", msg => {
  ...
  sellStock(event) {
    ...
    let sell_object = {
      ...
      stock_name: stock_name,
      qty: parseInt(stock_quantity),
      sold_at: this.props.stocksPrice[stock_id]
    }
    let player = this.props.playerNumber
    let send_object = {
      player: player,
    ...
    if (this.state.selected != "" && this.state.sell_qty != "") {
      this.channel.push("sell_request", {sell: send_object}).receive("ok", msg => {
        if (msg.status === "Invalid Quantity") {
          this.setState({sell_status: "Invalid quantity!"})
        }
        if (msg.status === "success") {
          this.setState({
            sell_status: "Successfully sold!"
          }, () => {
  ...
  sellSelect(event) {
    ...
    this.setState({
      stock_sell: this.props.stocksNames[event.target.value]
    })
    ...
    return (<div className="trade">
      <div className="trade-grid">
        <div className="timer-wallet">
          <div className="wallet-status">Money Left : ${
              this.props.playerNumber === 1
                ? this.props.player1.wallet
                : this.props.player2.wallet
            }</div>
        ...
        <div className="trade-operations">
          <form onSubmit={this.buyStock}>
            <div className="input-group">
              ...
              <input className="form-control" ref="stock_quantity" name="stock_quantity" type="number" placeholder={this.state.stock_name != ""
                  ? this.props.stocksQty[this.state.stock_id] + " stock(s) available"
                  : "Quantity"} onClick={this.handleQtyClick} onChange={this.handleStockQty}></input>
            </div>
        ...
        <div className="status-operations">
          <div className="buy-invoice">
            <p className="invoice-width">{this.state.stock_name}<span className="width-p5em">{
                  this.state.stock_name != "" && <span>
                      (${this.props.stocksPrice[this.state.stock_id]})<span>&nbsp;
                        x</span>
                    </span>
                }</span>
              <span className="width-p5em">{this.state.stock_name != "" && <span>{this.state.qty}</span>}<span className="width-p5em">{
                    this.state.stock_name != "" && <span>&nbsp;
                        =&nbsp;
                        <span className={this.props.stocksPrice[this.state.stock_id] * this.state.qty > this.props.player.wallet
                            ? "cannotbuy"
                            : "canbuy"}>${this.props.stocksPrice[this.state.stock_id] * this.state.qty}</span>
                      </span>
                  }</span>
              </span>
            </p>
          ...
          <form onSubmit={this.sellStock}>
            <div className="input-group">
              <select className="form-control" onChange={this.sellSelect}>
                <option default="default" value="">Select Stock</option>
                {
              ...
              <input className="form-control" ref="sell_quantity" name="sell_quantity" type="number" onClick={this.handleSellClickQty} placeholder={this.state.selected != ""
                  ? "You own " + this.state.stocks_qty[this.state.selected] + " stock(s)"
                  : "Quantity"} onChange={this.sellStockQty}></input>
            </div>
            <div className="height-p4em"></div>
        ...
        <div className="status-operations">
          <div className="sell-error">
            <p>{this.state.sell_status}</p>
          </div>
        </div>
        <div className="status-operations">
          <div className="sell-invoice">
            <p className="invoice-width">{this.state.stock_sell}<span className="width-p5em">{
                  this.state.stock_sell != "" && <span>
                      (${this.props.stocksPrice[this.state.selected]})<span>&nbsp;
                        x</span>
                    </span>
                }</span>
              <span className="width-p5em">{this.state.stock_sell != "" && <span>{this.state.sell_qty}</span>}<span className="width-p5em">{
                    this.state.stock_sell != "" && <span>&nbsp;
                        =&nbsp;
                        <span>${this.props.stocksPrice[this.state.selected] * this.state.sell_qty}</span>
                      </span>
                  }</span>
              </span>
            </p>
